  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (costingConfigurations.length > 0) {
        const sorted = costingConfigurations.filter((c) => currentFiscalYear - c.fiscalYearId >= 0 && currentFiscalYear - c.fiscalYearId <= 1).sort((a, b) => a.name.localeCompare(b.name));
